#!/usr/bin/env python3
"""
Test script for the Toronto Road Segment Crash Risk Prediction Pipeline

This script tests each component of the pipeline step by step to ensure
everything works correctly with the actual datasets.
"""
...
def test_spatial_join(collision_data, ksi_data, road_network):
    """Test spatial join step"""
    print("\n" + "=" * 60)
    print("STEP 2: Testing Spatial Join")
    print("=" * 60)
    
    try:
        from src.data_processing.spatial_join import perform_spatial_join
        
        segment_crashes = perform_spatial_join(collision_data, ksi_data, road_network)
        
        print(f"✅ Spatial join successful!")
    ...
    try:
        ...
        for feature in key_features:
            if feature in segment_features.columns:
                print(f"   - {feature} (mean): {segment_features[feature].mean():.4f}")
...
def main():
    ...
    print("🚀 Testing Toronto Road Segment Crash Risk Prediction Pipeline")
    print("=" * 80)
    ...
    # Step 2: Spatial Join
    segment_crashes = test_spatial_join(collision_data, ksi_data, road_network)
