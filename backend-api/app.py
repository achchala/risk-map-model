...
from src.models.model_trainer import ModelTrainer, TemporalCountModelTrainer  # type: ignore
...
# initialize legacy classifier model (kept for backward compatibility)
MODEL_PATH = PROJECT_ROOT / "outputs" / "models" / "toronto_risk_model.joblib"
model_trainer: ModelTrainer | None = ModelTrainer()

try:
    model_trainer.load_model(str(MODEL_PATH))
    logging.info("Legacy classifier model loaded successfully")
except Exception as e:
    logging.error(f"Failed to load legacy classifier model: {e}")
    model_trainer = None
...
def _compute_lambda_map_for_latest_window():
    ...
    This is used to annotate routing edges with expected crashes.
    """
    global lambda_per_hour_latest, latest_window_start
    ...
    logging.info(
        "Computed λ_per_hour for latest window %s for %d segments.",
        latest_window_start,
